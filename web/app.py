...
def _build_config() -> dict:
    ...
    config["llm_provider"] = st.session_state.get("llm_provider", "openai_compatible")
    # Model config: sidebar first, .env as fallback
    config["deep_think_llm"] = st.session_state.get("deep_think_llm") or os.getenv("DEEP_THINK_LLM", "deepseek-v4-pro")
    config["quick_think_llm"] = st.session_state.get("quick_think_llm") or os.getenv("QUICK_THINK_LLM", "deepseek-v4-flash")
    max_tokens_raw = (st.session_state.get("max_tokens") or "").strip()
    if max_tokens_raw:
    ...
    # API Key from .env (OPENAI_API_KEY for openai_compatible gateway)
