...
def get_stock_data(
    ...
    # Cross-validation: if mootdx returned fewer than 3 K-lines, try Sina fallback
    if len(df) < 3 and data_source == "mootdx (TCP)":
        logger.warning(
            "mootdx returned only %d K-lines for %s in date range, trying sina HTTP fallback",
            len(df), code,
        )
        try:
            df_sina = _sina_kline_fallback(code, start_date, end_date)
            if df_sina is not None and len(df_sina) >= 3:
                df = df_sina
                data_source = "sina HTTP (fallback, cross-validation)"
        except Exception:
            pass
...
def get_fundamentals(
    ...
    try:
        ...
        try:
            ...
            lines.append(f"[数据缺失: mootdx财务快照 - {e}]")
        ...
        try:
            ...
            lines.append(f"[数据缺失: 东方财富个股信息 - {e}]")
...
def get_margin_balance(
    ...
    来源：akshare（上交所/深交所两融统计）
    返回市场整体两融余额走势 + 个股所属市场的两融概况。
    """
    code = _normalize_ticker(ticker)
    market = "沪市" if code.startswith(("6", "9")) else "深市" if code.startswith(("0", "3")) else "北交所"
    lines = [
        f"# 两融余额数据 | {code}",
        f"# Source: 上交所/深交所官方",
        f"# Retrieved: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
    ...
    try:
        ...
        lines.append(f"## 市场整体两融概况 ({market})")
        if market == "沪市":
            df = ak.stock_margin_detail_sse(date=datetime.now().strftime("%Y%m%d"))
            lines.append(f"当日标的数: {len(df)} 只")
            lines.append(f"沪市全市场融资融券余额合计: {df['融资融券余额'].sum() / 1e8:.2f} 亿元")
            lines.append(f"沪市全市场融资余额: {df['融资余额'].sum() / 1e8:.2f} 亿元")
            lines.append(f"")
            lines.append("### 沪市两融数据明细 (前15只)")
            lines.append("代码 | 名称 | 融资余额 | 融券余额")
            for _, row in df.head(15).iterrows():
                code_row = str(row.get("标的证券代码", ""))
                name = str(row.get("标的证券简称", ""))
                fin_bal = float(row.get("融资余额", 0)) / 1e4
                sec_bal = float(row.get("融券余额", 0)) if "融券余额" in df.columns else 0
                lines.append(f"  {code_row} | {name} | {fin_bal:.1f}万 | {sec_bal:.1f}万")
        else:
            df = ak.stock_margin_detail_szse(date=datetime.now().strftime("%Y%m%d"))
            if len(df) > 0:
                lines.append(f"当日深市两融标的数: {len(df)} 只")
                lines.append(f"深市全市场融资买入额合计: {df['融资买入额'].sum() / 1e8:.2f} 亿元")
                lines.append(f"深市全市场融资余额合计: {df['融资余额'].sum() / 1e8:.2f} 亿元")
            else:
                lines.append("[数据提示: 深市两融数据非交易日无更新]")

        # 个股两融判断
        market_prefix = "沪" if market == "沪市" else "深"
        lines.append(f"")
        lines.append(f"### {code} 两融资格")
        lines.append(f"股票 {code} 为{market_prefix}市股票，是否可融资融券需以券商实时查询为准。")

    except ImportError:
        lines.append("[数据缺失: akshare 未安装，无法获取两融数据]")
    except Exception as e:
        logger.warning("Margin balance fetch failed for %s: %s", code, e)
        lines.append(f"[数据缺失: 两融接口异常 - {e}]")

    return "\n".join(lines)


# ===========================================================================
# 19. SHIBOR / Repo Rates
# ===========================================================================


def get_shibor() -> str:
    """获取 SHIBOR 利率及逆回购利率数据。

    来源：akshare（中国银行间市场 SHIBOR 报价）+ 回购定盘利率
    返回 SHIBOR 各期限（ON/1W/2W/1M/3M/6M/9M/1Y）及 FR001/FR007/FR014 回购定盘利率。
    """
    lines = [
        "# SHIBOR 利率数据",
        "# Source: 中国外汇交易中心 / 全国银行间同业拆借中心",
    ...
    try:
        ...
        if len(df) > 0:
            latest = df.tail(5)
            lines.append(f"## SHIBOR 利率 (近{len(latest)}个交易日)")
            lines.append("日期 | ON | 1W | 2W | 1M | 3M | 6M | 9M | 1Y")
            for _, row in latest.iterrows():
                date = str(row.get("日期", ""))[:10]
                on = row.get("O/N-定价", "N/A")
                w1 = row.get("1W-定价", "N/A")
                w2 = row.get("2W-定价", "N/A")
                m1 = row.get("1M-定价", "N/A")
                m3 = row.get("3M-定价", "N/A")
                m6 = row.get("6M-定价", "N/A")
                m9 = row.get("9M-定价", "N/A")
                y1 = row.get("1Y-定价", "N/A")
                lines.append(f"  {date}: ON={on}% 1W={w1}% 2W={w2}% 1M={m1}% 3M={m3}% 6M={m6}% 9M={m9}% 1Y={y1}%")

            on_current = latest.iloc[-1].get("O/N-定价", "N/A")
            on_chg = latest.iloc[-1].get("O/N-涨跌幅", "N/A")
            lines.append(f"")
            lines.append(f"当前隔夜SHIBOR: {on_current}% (日变动: {on_chg}bp)")
        else:
            lines.append("[数据缺失: 无SHIBOR数据]")

        # 逆回购定盘利率
        try:
            df_repo = ak.repo_rate_query()
            if len(df_repo) > 0:
                lines.append(f"")
                lines.append(f"## 回购定盘利率 (近{min(5, len(df_repo))}个交易日)")
                lines.append("日期 | FR001 | FR007 | FR014")
                for _, row in df_repo.tail(5).iterrows():
                    date = str(row.get("date", ""))[:10]
                    fr001 = row.get("FR001", "N/A")
                    fr007 = row.get("FR007", "N/A")
                    fr014 = row.get("FR014", "N/A")
                    lines.append(f"  {date}: FR001={fr001}% FR007={fr007}% FR014={fr014}%")

                latest_repo = df_repo.tail(1).iloc[-1]
                lines.append(f"")
                lines.append(f"当前FR007: {latest_repo.get('FR007', 'N/A')}% (银行间市场7天资金成本)")
                lines.append(f"当前FR001: {latest_repo.get('FR001', 'N/A')}% (隔夜资金成本)")
        except Exception as e:
            logger.warning("Repo rate fetch failed: %s", e)
            lines.append(f"[数据缺失: 逆回购利率接口异常 - {e}]")

    except ImportError:
        lines.append("[数据缺失: akshare 未安装，无法获取SHIBOR数据]")
    except Exception as e:
        logger.warning("SHIBOR fetch failed: %s", e)
        lines.append(f"[数据缺失: SHIBOR接口异常 - {e}]")

    return "\n".join(lines)


# ===========================================================================
# 20. Guba Sentiment (东方财富股吧讨论热度)
# ===========================================================================
...
def get_guba_sentiment(
    ...
    """获取东方财富股吧（guba.eastmoney.com）个股讨论热度数据。

    来源：东方财富股吧页面嵌入数据 (article_list JSON)
    返回帖子热度、最新热帖标题、发帖活跃度等信息。

    Args:
        ticker: A-stock code (e.g. 300750)
    Returns:
        str: 格式化的股吧讨论热度报告
    """
    code = _normalize_ticker(ticker)
    guba_code = f"{_get_prefix(code).upper()}{code}"
    lines = [
        f"# 东方财富股吧讨论热度 | {code}",
        f"# Source: 东方财富股吧 (guba.eastmoney.com)",
        f"# Retrieved: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
        "",
    ]
    ...
    try:
        r = _requests.get(
            f"https://guba.eastmoney.com/list,{guba_code}.html",
        ...
        # 提取 article_list 中嵌入的 JSON 数据
        match = _re.search(r'var article_list\s*=\s*(\{[^;]+});', html)
        if not match:
            lines.append("[数据缺失: 无法解析股吧数据]")
            return "\n".join(lines)
        ...
        if not articles:
            lines.append("[数据缺失: 股吧暂无帖子数据]")
            return "\n".join(lines)

        lines.append(f"## 股吧讨论概览")
        lines.append(f"  - 当前显示帖子数: {len(articles)} 条")
        lines.append(f"")

        # 统计特点
        lines.append(f"## 最新热帖 (前20条)")
        for i, art in enumerate(articles[:20], 1):
            title = art.get("post_title", "").strip()
            user = art.get("user_nickname", art.get("user_id", "匿名"))
            hits = art.get("post_hits", art.get("click_count", 0))
            comments = art.get("post_comments", art.get("comment_count", 0))
            if title:
                lines.append(f"  {i}. [{title[:60]}]")
                lines.append(f"     作者: {user} | 阅读: {hits} | 评论: {comments}")

        lines.append(f"")
        lines.append(f"## 情绪指标说明")
        lines.append(f"  东方财富股吧是A股最大的散户讨论社区，帖子标题中的关键词可以反映")
        lines.append(f"  市场情绪方向。近期热帖涵盖以下主题: ")

        # 提取关键词统计
        keywords = {"看多": 0, "看空": 0, "利好": 0, "利空": 0, "涨停": 0, "跌停": 0,
                     "买入": 0, "卖出": 0, "抄底": 0, "逃顶": 0, "风险": 0, "机会": 0}
        for art in articles:
            title = art.get("post_title", "")
            for kw in keywords:
                if kw in title:
                    keywords[kw] += 1

        pos_kws = keywords["看多"] + keywords["利好"] + keywords["买入"] + keywords["抄底"] + keywords["机会"] + keywords["涨停"]
        neg_kws = keywords["看空"] + keywords["利空"] + keywords["卖出"] + keywords["逃顶"] + keywords["风险"] + keywords["跌停"]
        sentiment_ratio = f"{pos_kws / max(neg_kws, 1):.1f}" if (pos_kws > 0 or neg_kws > 0) else "N/A"
        sentiment_label = "偏多" if pos_kws > neg_kws else "偏空" if neg_kws > pos_kws else "中性"
        lines.append(f"  - 标题关键词统计: 正面关键词 {pos_kws}次 / 负面关键词 {neg_kws}次")
        lines.append(f"  - 多空比: {sentiment_ratio} ({sentiment_label})")

    except Exception as e:
        logger.warning("Guba sentiment fetch failed for %s: %s", code, e)
        lines.append(f"[数据缺失: 股吧接口异常 - {e}]")

    return "\n".join(lines)


# ===========================================================================
# 21. Xueqiu Discussions (雪球讨论)
# ===========================================================================
...
def get_xueqiu_discussions(
    ...
    """获取雪球（xueqiu.com）平台个股讨论数据。

    来源：雪球股票页面 (xueqiu.com/S/{symbol}) + 递归解析
    返回雪球用户对该股票的近期讨论帖，包括热度、内容摘要等。
    雪球 API 需要登录态，此方法降级为页面内容分析。
    ...
    code = _normalize_ticker(ticker)
    prefix = "SH" if code.startswith(("6", "9")) else "SZ"
    symbol_id = f"{prefix}{code}"
    lines = [
        f"# 雪球讨论数据 | {code}",
        f"# Source: 雪球 (xueqiu.com)",
        f"# Retrieved: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
        "",
    ]
    ...
    try:
        ...
        # 尝试通过页面嵌入的股价数据 + 热门用户分析
        page_url = f"{_XUEQIU_BASE}/S/{symbol_id}"
        ...
        discussions = []
        # 查找页面中的讨论内容片段
        text_matches = _re.findall(r'\"text\":\"(.*?)\"', html)
        for t in text_matches:
            if len(t) > 10 and len(t) < 500:
                text_clean = t.replace("\\n", " ").replace("\\\"", "\"").replace("\\\\", "\\")
                discussions.append(text_clean)

        if discussions:
            lines.append(f"## 雪球讨论 ({len(discussions)} 条)")
            for i, text in enumerate(discussions[:10], 1):
                lines.append(f"  {i}. {text[:120]}")
        else:
            lines.append("[数据提示: 雪球页面数据需登录后访问，当前降级为股吧数据替代]")
            lines.append("")
            lines.append(f"雪球页面已加载成功，但讨论内容需要登录后才能显示。")
            lines.append(f"参考页面信息：{page_url}")
            lines.append(f"")
            lines.append(f"## 替代方案")
            lines.append(f"建议参考**股吧情绪分析**（get_guba_sentiment）获取散户讨论数据，")
            lines.append(f"以及**新闻分析**（get_news）获取机构观点的新闻报导。")

    except Exception as e:
        logger.warning("Xueqiu discussion fetch failed for %s: %s", code, e)
        lines.append(f"[数据缺失: 雪球接口异常 - {e}]")
