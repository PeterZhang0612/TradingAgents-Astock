...
def get_margin_balance(
    ...
    """
    Retrieve margin balance (两融余额) data for a stock.
    Shows margin buying, short selling balances, and recent changes.
    Uses the configured signal_data vendor.
    Args:
        ticker (str): A-stock code
    Returns:
...
def get_shibor() -> str:
    """
    Retrieve SHIBOR (Shanghai Interbank Offered Rate) and repo rate data.
    Shows SHIBOR rates across all tenors (ON, 1W, 2W, 1M, 3M, 6M, 9M, 1Y)
    and exchange repo rates (GC001, GC007, etc.).
    Uses the configured signal_data vendor.
    Args:
        None
    Returns:
        str: SHIBOR and repo rate report
    """
    return route_to_vendor("get_shibor")
