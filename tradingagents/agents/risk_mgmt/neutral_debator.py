...
- Northbound Flow as Smart Money Gauge: {northbound_flow} — Foreign institutional flow via Stock Connect is more informed than retail flow, but also more fickle — they exit faster than domestic funds. Use it as a confirming signal, not a primary thesis.
- Margin Balance as Leverage Barometer (两融余额): {margin_balance} — High margin balance indicates retail leverage is elevated, which amplifies both upside and downside. When margin balance is near historical highs, forced liquidation cascades become a systemic risk. Use it to calibrate position sizing.
